...
def gather_intended_incremental_index_by_ref(
    ...
    index_strategy=="incremental" docs only, so snapshot markers (which always have git.commit) are
    not double-counted.  Returns {} if the refs index doesn't exist."""
    out: dict[tuple[str, str, str, str], set[str]] = {}
    body = {"query": {"term": {"index_strategy": "incremental"}}}
...
def check_join_uniqueness(es: Elasticsearch, host: str, org: str, repo: str) -> list[str]:
    ...
    refs join doc. Split by content shape (no `index_strategy` on content docs):
    ...
    - Incremental (git.ref IS NOT NULL): each ref must resolve to EXACTLY ONE refs doc with
      `index_strategy == "incremental"` -- the anti-fan-out invariant for the surviving join.
        ...
        try:
            resp = es.search(
                ...
                query={"bool": {"filter": [
                    ...
                    {"term": {"index_strategy": "incremental"}},
                ]}},
