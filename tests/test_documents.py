...
def _set_worker_ctx(host: str, org: str, repo: str, commit_sha: str, repo_dir, symlink_paths=frozenset()) -> None:
    ...
    documents._WORKER_CTX.update(
        ...
        symlink_paths=symlink_paths, strategy="snapshot",
    )
...
def _set_worker_ctx_incremental(host: str, org: str, repo: str, ref: str, repo_dir, symlink_paths=frozenset()) -> None:
    documents._WORKER_CTX.update(
        ...
        symlink_paths=symlink_paths, strategy="incremental",
    )
...
class TestBuildFileDoc:
    def test_nested_dir(self, tmp_path):
        p = tmp_path / "a.txt"
        p.write_text("hello")
        _id, doc = build_file_doc("github", "acme", "widgets", "deadbeef", "src/pkg/a.txt", p)
    ...
    def test_root_file_has_empty_directory(self, tmp_path):
        p = tmp_path / "README"
        p.write_text("hi")
        _id, doc = build_file_doc("github", "acme", "widgets", "deadbeef", "README", p)
        assert doc["file"]["directory"] == ""

    def test_no_extension_is_none(self, tmp_path):
        p = tmp_path / "Makefile"
        p.write_text("all:")
        _id, doc = build_file_doc("github", "acme", "widgets", "deadbeef", "Makefile", p)
    ...
    def test_deterministic_id(self, tmp_path):
        p = tmp_path / "a.txt"
        p.write_text("hello")
        id1, _ = build_file_doc("github", "acme", "widgets", "deadbeef", "a.txt", p)
        id2, _ = build_file_doc("github", "acme", "widgets", "deadbeef", "a.txt", p)
        assert id1 == id2
    ...
    def test_host_changes_id(self, tmp_path):
        p = tmp_path / "a.txt"
        p.write_text("hello")
        id_gh, _ = build_file_doc("github", "acme", "widgets", "deadbeef", "a.txt", p)
        id_gl, _ = build_file_doc("gitlab", "acme", "widgets", "deadbeef", "a.txt", p)
        assert id_gh != id_gl
    ...
    def test_binary_flag_sets_binary_attribute(self, tmp_path):
        ...
        _id, doc = build_file_doc("github", "acme", "widgets", "deadbeef", "a.bin", p, binary=True)
    ...
    def test_symlink_sets_target_path_and_target_size(self, tmp_path):
        ...
        _id, doc = build_file_doc("github", "acme", "widgets", "deadbeef", "link.txt", link)
    ...
    def test_broken_symlink_has_target_path_but_no_target_size(self, tmp_path):
        ...
        _id, doc = build_file_doc("github", "acme", "widgets", "deadbeef", "dangling.txt", link)
...
class TestIterLineDocs:
    ...
    def test_empty_content_yields_no_docs(self):
        assert list(iter_line_docs("github", "acme", "widgets", "deadbeef", "a.txt", "")) == []
