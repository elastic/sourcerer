...
class TestBaseCommitAvailable:
    def test_true_for_present_commit(self, repo):
        (repo / "a.txt").write_text("hi\n")
        sha = _commit_all(repo, "init")
        assert base_commit_available(repo, sha) is True

    def test_false_for_absent_commit(self, repo):
        (repo / "a.txt").write_text("hi\n")
...
class TestPlanChangesIntegration:
    def test_base_missing_returns_flagged_plan(self, repo):
        (repo / "a.txt").write_text("hi\n")
    ...
    def test_type_change_file_to_symlink(self, repo):
    ...
    def test_awkward_paths_preserved(self, repo):
...
class TestParseNameStatusZ:
    def test_copy_indexes_destination_only(self):
        raw = b"C100\x00src.txt\x00dst.txt\x00"
        delete, index = _parse_name_status_z(raw)
        assert delete == []
        assert index == ["dst.txt"]

    def test_rename_deletes_source_indexes_destination(self):
    ...
    def test_truncated_rename_record_ignored(self):
