...
1. Install the `sourcerer` CLI:
   ```sh
   uv tool install "git+https://github.com/elastic/sourcerer.git@v3.0.1"
   ```
...
To upgrade, reinstall from the desired release tag, replacing `v3.0.1` with the release you want:

```sh
uv tool install --reinstall "git+https://github.com/elastic/sourcerer.git@v3.0.1"
```
...
```sh
./scripts/release.sh prepare v3.0.1
```
...
```sh
./scripts/release.sh publish v3.0.1
```
