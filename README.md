...
1. Install the `sourcerer` CLI:
   ```sh
   uv tool install "git+https://github.com/elastic/sourcerer.git@v2.4.2"
   ```
...
To upgrade, reinstall from the desired release tag, replacing `v2.4.2` with the release you want:

```sh
uv tool install --reinstall "git+https://github.com/elastic/sourcerer.git@v2.4.2"
```
...
```sh
./scripts/release.sh prepare v2.4.2
```
...
```sh
./scripts/release.sh publish v2.4.2
```
